...
public class Sudoku{
	...
	public static void main(String[] args) {
		...
		do{
			...
			if(errore)
				System.out.print("\n\tAttenzione! scelta errata.\n\tScegli nuovamente la difficoltà: ");
		}while(errore);
	...
	private static boolean controlloArray(int[][] array, int riga, int colonna) {	
		...
		if(array[riga][colonna] == 0)//-------------------------------------------Evita il controllo se la cella è vuota o viene svuotata
			return false;
	...
	private static void svuotaArray(int[][] array, int difficolta) {
		for(int indice = 0; indice < difficolta; indice++) {
			...
			if(array[riga][colonna] == 0)//-----------------------------------------Controllo per evitare di svuotare una cella già vuota
				indice--;
	...
	private static boolean inputUtente(int[][] array, int[][] locked) {
		...
		do {
			...
			if(locked[riga][colonna] != 0) {//------------------------------------Controllo se la cella è modificabile
		...
		}while(erroreInput);//----------------------------------------------------Ciclo di controllo inserimento già presente
	terminato = controlloTerminato(array);
	return terminato;
	}

	private static boolean controlloTerminato(int[][] array) {
		for (int i = 0; i < DIM; i++)//-------------------------------------------Ciclo annidato di controllo: se l'array è completo ritorna vero
			for (int j = 0; j < DIM; j++) {
				if(array[i][j] == 0)
					return false;
			}
